...
def recommend():
    ...
    try:
        ...
        price_range = (
            (price_min, price_max) if price_min > 0 or price_max < 999 else None
        )
        ...
        if not steam_input.isdigit():
            ...
            if not steam_id:
                return jsonify(
                    {
                        "error": "Could not find Steam user. Make sure your profile is public and the username is correct!"
                    }
                )

        # Get recommendations
        results = recommender.get_recommendations(
            steam_id, min_rating, sort_by, price_range, show_recent_only
        )
